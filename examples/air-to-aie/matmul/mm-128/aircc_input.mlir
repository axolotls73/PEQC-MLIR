        ...
        %15 = air.herd @herd_0 async [%async_token_14]  tile (%arg10, %arg11) in (%arg12=%c2_13, %arg13=%c2_13) attributes {id = 3 : i32} {
          ...
          %17:4 = scf.for %arg14 = %c0_24 to %c128_25 step %c64_23 iter_args(%arg15 = %async_token_33, %arg16 = %async_token_35, %arg17 = %async_token_35, %arg18 = %async_token_35) -> (!air.async.token, !air.async.token, !air.async.token, !air.async.token) {
            %19 = affine.if #set()[%arg10, %arg11] -> !air.async.token {
              %24 = air.channel.get async [%arg18, %arg15, %async_token_33]  @channel_0[%arg10, %arg11] (%results_34[] [] []) {id = 11 : i32} : (memref<32x32xi32, 2>)
              affine.yield %24 : !air.async.token
            } else {
              %24 = air.channel.get async [%arg18, %arg15, %async_token_33]  @channel_1[%arg10, %arg11] (%results_34[] [] []) {id = 12 : i32} : (memref<32x32xi32, 2>)
              affine.yield %24 : !air.async.token
            }
            %20 = affine.if #set1()[%arg10, %arg11] -> !air.async.token {
              %24 = air.channel.get async [%arg18, %arg15, %async_token_35]  @channel_2[%arg10, %arg11] (%results_36[] [] []) {id = 13 : i32} : (memref<32x32xi32, 2>)
              affine.yield %24 : !air.async.token
            } else {
              %24 = air.channel.get async [%arg18, %arg15, %async_token_35]  @channel_3[%arg10, %arg11] (%results_36[] [] []) {id = 14 : i32} : (memref<32x32xi32, 2>)
              affine.yield %24 : !air.async.token
            }
            %async_token_38 = air.execute [%arg17, %20, %19] {
              linalg.generic {indexing_maps = [#map1, #map2, #map3], iterator_types = ["parallel", "parallel", "reduction"]} ins(%results_34, %results_36 : memref<32x32xi32, 2>, memref<32x32xi32, 2>) outs(%results_27 : memref<32x32xi32, 2>) {
            ...
            %21 = affine.if #set()[%arg10, %arg11] -> !air.async.token {
              %24 = air.channel.get async [%20, %19, %arg16]  @channel_0[%arg10, %arg11] (%results_32[] [] []) {id = 11 : i32} : (memref<32x32xi32, 2>)
              affine.yield %24 : !air.async.token
            } else {
              %24 = air.channel.get async [%20, %19, %arg16]  @channel_1[%arg10, %arg11] (%results_32[] [] []) {id = 12 : i32} : (memref<32x32xi32, 2>)
              affine.yield %24 : !air.async.token
            }
            %22 = affine.if #set1()[%arg10, %arg11] -> !air.async.token {
              %24 = air.channel.get async [%20, %19, %arg16]  @channel_2[%arg10, %arg11] (%results_30[] [] []) {id = 13 : i32} : (memref<32x32xi32, 2>)
              affine.yield %24 : !air.async.token
            } else {
              %24 = air.channel.get async [%20, %19, %arg16]  @channel_3[%arg10, %arg11] (%results_30[] [] []) {id = 14 : i32} : (memref<32x32xi32, 2>)
              affine.yield %24 : !air.async.token
            }
            %async_token_41 = air.execute [%async_token_38, %22, %21] {
              linalg.generic {indexing_maps = [#map1, #map2, #map3], iterator_types = ["parallel", "parallel", "reduction"]} ins(%results_32, %results_30 : memref<32x32xi32, 2>, memref<32x32xi32, 2>) outs(%results_27 : memref<32x32xi32, 2>) {
              ^bb0(%in: i32, %in_44: i32, %out: i32):
                %24 = arith.muli %in, %in_44 : i32
                %25 = arith.addi %out, %24 : i32
                linalg.yield %25 : i32
