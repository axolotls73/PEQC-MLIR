...
argparser.add_argument('dir', type=str, # nargs='+',
    help='')
argparser.add_argument('outdir', type=str, # nargs='+',
    help='')
...
cw.writerow([
  ...
  'num_stmts_file1', 'num_stmts_file2', 'num_tasks_file1', 'num_tasks_file2', 'num_cdagnodes_f1', 'num_cdagnodes_f2',
  'memcum', 'memused_f1', 'memused_f2',
  'command'
])
  ...
  for file1, file2, name, liveout in pairs:
    ...
    if stdout and not timeout:
      try:
        ...
        num_cdagnodes_f1, =  re.search(r'Number of nodes in CDAGs for P1: total=(\d+)', stdout).groups()
        num_cdagnodes_f2, =  re.search(r'Number of nodes in CDAGs for P2: total=(\d+)', stdout).groups()
        num_workers_f1, num_workers_f2 = re.findall(r'(\d+) maximum concurrent workers', stdout)
        memcum, = re.findall(r'Approximate cumulative memory size: (\d+) Bytes', stdout)
        memused_f1, memused_f2 = re.findall(r'(\d+) memory bytes used', stdout)
      except Exception:
        interp_f1 = interp_f2 = equivalence = num_stmts_f1 = num_stmts_f2 = num_workers_f1 = num_workers_f2 = num_cdagnodes_f1 = num_cdagnodes_f2 = memcum = memused_f1 = memused_f2 = 'N/A'
    else:
        interp_f1 = interp_f2 = equivalence = num_stmts_f1 = num_stmts_f2 = num_workers_f1 = num_workers_f2 = num_cdagnodes_f1 = num_cdagnodes_f2 = memcum = memused_f1 = memused_f2 = 'N/A'
    ...
    cw.writerow([
      ...
      num_stmts_f1, num_stmts_f2, num_workers_f1, num_workers_f2, num_cdagnodes_f1, num_cdagnodes_f2,
      memcum, memused_f1, memused_f2,
      command
    ])
    statsfile.flush()
...
benchdir = args.dir
outdir = f'{topdir}/output_{check_suffix}'
...
checkpairs(pairs, outdir, args.dir)
