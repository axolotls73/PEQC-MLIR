module {
  func.func @kernel_durbin(%arg0: i32, %arg1: memref<?xf64>, %arg2: memref<?xf64>) {
    %c42 = arith.constant 42 : index
    %c-1 = arith.constant -1 : index
    %c40 = arith.constant 40 : index
    %c1 = arith.constant 1 : index
    %c0 = arith.constant 0 : index
    %0 = llvm.mlir.undef : f64
    %cst = arith.constant 0.000000e+00 : f64
    %cst_0 = arith.constant 1.000000e+00 : f64
    %alloca = memref.alloca() : memref<f64>
    memref.store %0, %alloca[] : memref<f64>
    %alloca_1 = memref.alloca() : memref<f64>
    memref.store %0, %alloca_1[] : memref<f64>
    %alloca_2 = memref.alloca() : memref<f64>
    memref.store %0, %alloca_2[] : memref<f64>
    %alloca_3 = memref.alloca() : memref<40xf64>
    %1 = memref.load %arg1[%c0] : memref<?xf64>
    %2 = arith.negf %1 : f64
    memref.store %2, %arg2[%c0] : memref<?xf64>
    memref.store %cst_0, %alloca_1[] : memref<f64>
    %3 = memref.load %arg1[%c0] : memref<?xf64>
    %4 = arith.negf %3 : f64
    memref.store %4, %alloca_2[] : memref<f64>
    scf.for %arg3 = %c1 to %c40 step %c1 {
      %5 = memref.load %alloca_2[] : memref<f64>
      %6 = arith.mulf %5, %5 : f64
      %7 = arith.subf %cst_0, %6 : f64
      %8 = memref.load %alloca_1[] : memref<f64>
      %9 = arith.mulf %7, %8 : f64
      memref.store %9, %alloca_1[] : memref<f64>
      memref.store %cst, %alloca[] : memref<f64>
      scf.for %arg4 = %c0 to %arg3 step %c1 {
        %19 = arith.muli %arg4, %c-1 overflow<nsw> : index
        %20 = arith.addi %arg3, %19 : index
        %21 = arith.addi %20, %c-1 : index
        %22 = memref.load %arg1[%21] : memref<?xf64>
        %23 = memref.load %arg2[%arg4] : memref<?xf64>
        %24 = arith.mulf %22, %23 : f64
        %25 = memref.load %alloca[] : memref<f64>
        %26 = arith.addf %25, %24 : f64
        memref.store %26, %alloca[] : memref<f64>
      }
      %10 = memref.load %arg1[%arg3] : memref<?xf64>
      %11 = memref.load %alloca[] : memref<f64>
      %12 = arith.addf %10, %11 : f64
      %13 = arith.negf %12 : f64
      %14 = arith.divf %13, %9 : f64
      memref.store %14, %alloca_2[] : memref<f64>
      %15 = async.create_group %c42 : !async.group
      %16 = scf.for %arg4 = %c0 to %arg3 step %c1 iter_args(%arg5 = %c0) -> (index) {
        %token = async.execute {
          %21 = memref.load %arg2[%arg4] : memref<?xf64>
          %22 = arith.muli %arg4, %c-1 overflow<nsw> : index
          %23 = arith.addi %arg3, %22 : index
          %24 = arith.addi %23, %c-1 : index
          %25 = memref.load %arg2[%24] : memref<?xf64>
          %26 = arith.mulf %14, %25 : f64
          %27 = arith.addf %21, %26 : f64
          memref.store %27, %alloca_3[%arg4] : memref<40xf64>
          async.yield
        }
        %19 = async.add_to_group %token, %15 : !async.token
        %20 = arith.addi %arg5, %c1 : index
        scf.yield %20 : index
      }
      async.await_all %15
      %17 = async.create_group %c42 : !async.group
      %18 = scf.for %arg4 = %c0 to %arg3 step %c1 iter_args(%arg5 = %c0) -> (index) {
        %token = async.execute {
          %21 = memref.load %alloca_3[%arg4] : memref<40xf64>
          memref.store %21, %arg2[%arg4] : memref<?xf64>
          async.yield
        }
        %19 = async.add_to_group %token, %17 : !async.token
        %20 = arith.addi %arg5, %c1 : index
        scf.yield %20 : index
      }
      async.await_all %17
      memref.store %14, %arg2[%arg3] : memref<?xf64>
    }
    return
  }
}

