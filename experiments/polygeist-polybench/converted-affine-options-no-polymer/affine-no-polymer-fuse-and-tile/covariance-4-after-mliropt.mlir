#map = affine_map<(d0) -> (d0)>
#map1 = affine_map<(d0) -> (d0 + 28)>
#map2 = affine_map<(d0) -> (d0 + 32)>
#map3 = affine_map<(d0) -> (-d0 + 28)>
module {
  func.func @kernel_covariance(%arg0: i32, %arg1: i32, %arg2: f64, %arg3: memref<?x28xf64>, %arg4: memref<?x28xf64>, %arg5: memref<?xf64>) {
    %alloc = memref.alloc() : memref<1xf64>
    %cst = arith.constant 1.000000e+00 : f64
    %cst_0 = arith.constant 0.000000e+00 : f64
    affine.for %arg6 = 0 to 28 step 32 {
      affine.for %arg7 = #map(%arg6) to #map1(%arg6) {
        affine.store %cst_0, %arg5[%arg7] : memref<?xf64>
        affine.for %arg8 = 0 to 32 {
          %3 = affine.load %arg3[%arg8, %arg7] : memref<?x28xf64>
          %4 = affine.load %arg5[%arg7] : memref<?xf64>
          %5 = arith.addf %4, %3 : f64
          affine.store %5, %arg5[%arg7] : memref<?xf64>
        }
        %1 = affine.load %arg5[%arg7] : memref<?xf64>
        %2 = arith.divf %1, %arg2 : f64
        affine.store %2, %arg5[%arg7] : memref<?xf64>
      }
    }
    affine.for %arg6 = 0 to 32 step 32 {
      affine.for %arg7 = 0 to 28 step 32 {
        affine.for %arg8 = #map(%arg6) to #map2(%arg6) {
          affine.for %arg9 = #map(%arg7) to #map1(%arg7) {
            affine.store %cst_0, %alloc[0] : memref<1xf64>
            affine.for %arg10 = 0 to 32 {
              %6 = affine.load %arg3[%arg10, %arg9] : memref<?x28xf64>
              %7 = affine.load %alloc[0] : memref<1xf64>
              %8 = arith.addf %7, %6 : f64
              affine.store %8, %alloc[0] : memref<1xf64>
            }
            %1 = affine.load %alloc[0] : memref<1xf64>
            %2 = arith.divf %1, %arg2 : f64
            affine.store %2, %alloc[0] : memref<1xf64>
            %3 = affine.load %alloc[0] : memref<1xf64>
            %4 = affine.load %arg3[%arg8, %arg9] : memref<?x28xf64>
            %5 = arith.subf %4, %3 : f64
            affine.store %5, %arg3[%arg8, %arg9] : memref<?x28xf64>
          }
        }
      }
    }
    %0 = arith.subf %arg2, %cst : f64
    affine.for %arg6 = 0 to 28 {
      affine.for %arg7 = 0 to #map3(%arg6) {
        affine.store %cst_0, %arg4[%arg6, %arg6 + %arg7] : memref<?x28xf64>
        affine.for %arg8 = 0 to 32 {
          %3 = affine.load %arg3[%arg8, %arg6] : memref<?x28xf64>
          %4 = affine.load %arg3[%arg8, %arg6 + %arg7] : memref<?x28xf64>
          %5 = arith.mulf %3, %4 : f64
          %6 = affine.load %arg4[%arg6, %arg6 + %arg7] : memref<?x28xf64>
          %7 = arith.addf %6, %5 : f64
          affine.store %7, %arg4[%arg6, %arg6 + %arg7] : memref<?x28xf64>
        }
        %1 = affine.load %arg4[%arg6, %arg6 + %arg7] : memref<?x28xf64>
        %2 = arith.divf %1, %0 : f64
        affine.store %2, %arg4[%arg6, %arg6 + %arg7] : memref<?x28xf64>
        affine.store %2, %arg4[%arg6 + %arg7, %arg6] : memref<?x28xf64>
      }
    }
    return
  }
}

