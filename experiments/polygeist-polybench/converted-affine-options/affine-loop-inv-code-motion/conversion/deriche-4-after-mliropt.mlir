#map = affine_map<(d0) -> (d0 * 32)>
#map1 = affine_map<(d0) -> (d0 * 32 + 32)>
module {
  func.func @kernel_deriche(%arg0: i32, %arg1: i32, %arg2: f32, %arg3: memref<?x64xf32>, %arg4: memref<?x64xf32>, %arg5: memref<?x64xf32>, %arg6: memref<?x64xf32>) {
    %cst = arith.constant 0.000000e+00 : f32
    %0 = llvm.mlir.undef : f32
    %cst_0 = arith.constant 1.000000e+00 : f32
    %cst_1 = arith.constant 2.000000e+00 : f32
    %cst_2 = arith.constant -2.000000e+00 : f32
    %alloca = memref.alloca() : memref<f32>
    %alloca_3 = memref.alloca() : memref<f32>
    %alloca_4 = memref.alloca() : memref<f32>
    %alloca_5 = memref.alloca() : memref<f32>
    %alloca_6 = memref.alloca() {scop.scratchpad} : memref<1xf32>
    %alloca_7 = memref.alloca() {scop.scratchpad} : memref<1xf32>
    %alloca_8 = memref.alloca() : memref<1xf32>
    %alloca_9 = memref.alloca() : memref<1xf32>
    %alloca_10 = memref.alloca() : memref<f32>
    %alloca_11 = memref.alloca() {scop.scratchpad} : memref<1xf32>
    %alloca_12 = memref.alloca() {scop.scratchpad} : memref<1xf32>
    %alloca_13 = memref.alloca() : memref<1xf32>
    %alloca_14 = memref.alloca() : memref<f32>
    %alloca_15 = memref.alloca() {scop.scratchpad} : memref<1xf32>
    %alloca_16 = memref.alloca() : memref<1xf32>
    %alloca_17 = memref.alloca() : memref<1xf32>
    %alloca_18 = memref.alloca() {scop.scratchpad} : memref<1xf32>
    %alloca_19 = memref.alloca() : memref<1xf32>
    %alloca_20 = memref.alloca() : memref<1xf32>
    %alloca_21 = memref.alloca() : memref<f32>
    %alloca_22 = memref.alloca() : memref<f32>
    %alloca_23 = memref.alloca() : memref<f32>
    %alloca_24 = memref.alloca() : memref<1xf32>
    %alloca_25 = memref.alloca() : memref<f32>
    %1 = arith.mulf %arg2, %cst_2 : f32
    %2 = math.exp %1 : f32
    %3 = arith.negf %2 : f32
    affine.store %3, %alloca_6[0] : memref<1xf32>
    %4 = arith.negf %arg2 : f32
    %5 = math.powf %cst_1, %4 : f32
    affine.store %5, %alloca_15[0] : memref<1xf32>
    %6 = arith.negf %arg2 : f32
    %7 = math.exp %6 : f32
    %8 = arith.mulf %arg2, %7 : f32
    %9 = arith.subf %cst_0, %7 : f32
    %10 = arith.mulf %9, %9 : f32
    %11 = arith.mulf %8, %cst_1 : f32
    %12 = arith.addf %11, %cst_0 : f32
    %13 = arith.mulf %arg2, %cst_1 : f32
    %14 = math.exp %13 : f32
    %15 = arith.subf %12, %14 : f32
    %16 = arith.divf %10, %15 : f32
    affine.store %16, %alloca_11[0] : memref<1xf32>
    %17 = affine.load %alloca_11[0] : memref<1xf32>
    affine.store %17, %alloca_24[0] : memref<1xf32>
    %18 = arith.negf %arg2 : f32
    %19 = math.exp %18 : f32
    %20 = arith.mulf %17, %19 : f32
    affine.store %20, %alloca_8[0] : memref<1xf32>
    %21 = arith.subf %arg2, %cst_0 : f32
    %22 = arith.mulf %20, %21 : f32
    affine.store %22, %alloca_18[0] : memref<1xf32>
    %23 = affine.load %alloca_24[0] : memref<1xf32>
    %24 = arith.negf %23 : f32
    %25 = arith.mulf %arg2, %cst_2 : f32
    %26 = math.exp %25 : f32
    %27 = arith.mulf %24, %26 : f32
    affine.store %27, %alloca_7[0] : memref<1xf32>
    %28 = affine.load %alloca_8[0] : memref<1xf32>
    %29 = arith.addf %arg2, %cst_0 : f32
    %30 = arith.mulf %28, %29 : f32
    affine.store %30, %alloca_12[0] : memref<1xf32>
    affine.store %0, %alloca_5[] : memref<f32>
    affine.store %0, %alloca_14[] : memref<f32>
    affine.store %0, %alloca_4[] : memref<f32>
    affine.store %0, %alloca_25[] : memref<f32>
    %31 = affine.load %alloca_11[0] : memref<1xf32>
    %32 = affine.load %alloca_18[0] : memref<1xf32>
    %33 = affine.load %alloca_15[0] : memref<1xf32>
    %34 = affine.load %alloca_6[0] : memref<1xf32>
    affine.for %arg7 = 0 to 64 {
      affine.store %cst, %alloca_5[] : memref<f32>
      affine.store %cst, %alloca_25[] : memref<f32>
      affine.store %cst, %alloca_4[] : memref<f32>
      affine.for %arg8 = 0 to 64 {
        %47 = affine.load %arg3[%arg7, %arg8] : memref<?x64xf32>
        %48 = arith.mulf %31, %47 : f32
        %49 = affine.load %alloca_5[] : memref<f32>
        %50 = arith.mulf %32, %49 : f32
        %51 = arith.addf %48, %50 : f32
        %52 = affine.load %alloca_4[] : memref<f32>
        affine.store %52, %alloca_9[0] : memref<1xf32>
        %53 = arith.mulf %33, %52 : f32
        %54 = arith.addf %51, %53 : f32
        %55 = affine.load %alloca_25[] : memref<f32>
        %56 = arith.mulf %34, %55 : f32
        %57 = arith.addf %54, %56 : f32
        affine.store %57, %arg5[%arg7, %arg8] : memref<?x64xf32>
        %58 = affine.load %arg5[%arg7, %arg8] : memref<?x64xf32>
        affine.store %58, %alloca_4[] : memref<f32>
        %59 = affine.load %alloca_9[0] : memref<1xf32>
        affine.store %59, %alloca_25[] : memref<f32>
        %60 = affine.load %arg3[%arg7, %arg8] : memref<?x64xf32>
        affine.store %60, %alloca_5[] : memref<f32>
      }
    }
    affine.store %0, %alloca_3[] : memref<f32>
    affine.store %0, %alloca_21[] : memref<f32>
    affine.store %0, %alloca_10[] : memref<f32>
    affine.store %0, %alloca_23[] : memref<f32>
    affine.store %0, %alloca_22[] : memref<f32>
    affine.store %0, %alloca[] : memref<f32>
    %35 = affine.load %alloca_12[0] : memref<1xf32>
    %36 = affine.load %alloca_7[0] : memref<1xf32>
    %37 = affine.load %alloca_15[0] : memref<1xf32>
    %38 = affine.load %alloca_6[0] : memref<1xf32>
    affine.for %arg7 = 0 to 64 {
      affine.store %cst, %alloca_21[] : memref<f32>
      affine.store %cst, %alloca_3[] : memref<f32>
      affine.store %cst, %alloca[] : memref<f32>
      affine.store %cst, %alloca_22[] : memref<f32>
      affine.for %arg8 = 0 to 64 {
        %47 = affine.load %alloca_3[] : memref<f32>
        affine.store %47, %alloca_13[0] : memref<1xf32>
        %48 = arith.mulf %35, %47 : f32
        %49 = affine.load %alloca_21[] : memref<f32>
        %50 = arith.mulf %36, %49 : f32
        %51 = arith.addf %48, %50 : f32
        %52 = affine.load %alloca_22[] : memref<f32>
        affine.store %52, %alloca_16[0] : memref<1xf32>
        %53 = arith.mulf %37, %52 : f32
        %54 = arith.addf %51, %53 : f32
        %55 = affine.load %alloca[] : memref<f32>
        %56 = arith.mulf %38, %55 : f32
        %57 = arith.addf %54, %56 : f32
        affine.store %57, %arg6[%arg7, -%arg8 + 63] : memref<?x64xf32>
        %58 = affine.load %arg6[%arg7, -%arg8 + 63] : memref<?x64xf32>
        affine.store %58, %alloca_22[] : memref<f32>
        %59 = affine.load %alloca_16[0] : memref<1xf32>
        affine.store %59, %alloca[] : memref<f32>
        %60 = affine.load %arg3[%arg7, -%arg8 + 63] : memref<?x64xf32>
        affine.store %60, %alloca_3[] : memref<f32>
        %61 = affine.load %alloca_13[0] : memref<1xf32>
        affine.store %61, %alloca_21[] : memref<f32>
      }
    }
    affine.for %arg7 = 0 to 2 {
      affine.for %arg8 = 0 to 2 {
        affine.for %arg9 = #map(%arg7) to #map1(%arg7) {
          affine.for %arg10 = #map(%arg8) to #map1(%arg8) {
            %47 = affine.load %arg5[%arg9, %arg10] : memref<?x64xf32>
            %48 = affine.load %arg6[%arg9, %arg10] : memref<?x64xf32>
            %49 = arith.addf %47, %48 : f32
            affine.store %49, %arg4[%arg9, %arg10] : memref<?x64xf32>
          }
        }
      }
    }
    %39 = affine.load %alloca_11[0] : memref<1xf32>
    %40 = affine.load %alloca_18[0] : memref<1xf32>
    %41 = affine.load %alloca_15[0] : memref<1xf32>
    %42 = affine.load %alloca_6[0] : memref<1xf32>
    affine.for %arg7 = 0 to 64 {
      affine.store %cst, %alloca_25[] : memref<f32>
      affine.store %cst, %alloca_4[] : memref<f32>
      affine.store %cst, %alloca_14[] : memref<f32>
      affine.for %arg8 = 0 to 64 {
        %47 = affine.load %arg4[%arg8, %arg7] : memref<?x64xf32>
        %48 = arith.mulf %39, %47 : f32
        %49 = affine.load %alloca_14[] : memref<f32>
        %50 = arith.mulf %40, %49 : f32
        %51 = arith.addf %48, %50 : f32
        %52 = affine.load %alloca_4[] : memref<f32>
        affine.store %52, %alloca_19[0] : memref<1xf32>
        %53 = arith.mulf %41, %52 : f32
        %54 = arith.addf %51, %53 : f32
        %55 = affine.load %alloca_25[] : memref<f32>
        %56 = arith.mulf %42, %55 : f32
        %57 = arith.addf %54, %56 : f32
        affine.store %57, %arg5[%arg8, %arg7] : memref<?x64xf32>
        %58 = affine.load %arg5[%arg8, %arg7] : memref<?x64xf32>
        affine.store %58, %alloca_4[] : memref<f32>
        %59 = affine.load %alloca_19[0] : memref<1xf32>
        affine.store %59, %alloca_25[] : memref<f32>
        %60 = affine.load %arg4[%arg8, %arg7] : memref<?x64xf32>
        affine.store %60, %alloca_14[] : memref<f32>
      }
    }
    %43 = affine.load %alloca_12[0] : memref<1xf32>
    %44 = affine.load %alloca_7[0] : memref<1xf32>
    %45 = affine.load %alloca_15[0] : memref<1xf32>
    %46 = affine.load %alloca_6[0] : memref<1xf32>
    affine.for %arg7 = 0 to 64 {
      affine.store %cst, %alloca[] : memref<f32>
      affine.store %cst, %alloca_22[] : memref<f32>
      affine.store %cst, %alloca_23[] : memref<f32>
      affine.store %cst, %alloca_10[] : memref<f32>
      affine.for %arg8 = 0 to 64 {
        %47 = affine.load %alloca_10[] : memref<f32>
        affine.store %47, %alloca_17[0] : memref<1xf32>
        %48 = arith.mulf %43, %47 : f32
        %49 = affine.load %alloca_23[] : memref<f32>
        %50 = arith.mulf %44, %49 : f32
        %51 = arith.addf %48, %50 : f32
        %52 = affine.load %alloca_22[] : memref<f32>
        affine.store %52, %alloca_20[0] : memref<1xf32>
        %53 = arith.mulf %45, %52 : f32
        %54 = arith.addf %51, %53 : f32
        %55 = affine.load %alloca[] : memref<f32>
        %56 = arith.mulf %46, %55 : f32
        %57 = arith.addf %54, %56 : f32
        affine.store %57, %arg6[-%arg8 + 63, %arg7] : memref<?x64xf32>
        %58 = affine.load %arg6[-%arg8 + 63, %arg7] : memref<?x64xf32>
        affine.store %58, %alloca_22[] : memref<f32>
        %59 = affine.load %alloca_20[0] : memref<1xf32>
        affine.store %59, %alloca[] : memref<f32>
        %60 = affine.load %arg4[-%arg8 + 63, %arg7] : memref<?x64xf32>
        affine.store %60, %alloca_10[] : memref<f32>
        %61 = affine.load %alloca_17[0] : memref<1xf32>
        affine.store %61, %alloca_23[] : memref<f32>
      }
    }
    affine.for %arg7 = 0 to 2 {
      affine.for %arg8 = 0 to 2 {
        affine.for %arg9 = #map(%arg7) to #map1(%arg7) {
          affine.for %arg10 = #map(%arg8) to #map1(%arg8) {
            %47 = affine.load %arg5[%arg9, %arg10] : memref<?x64xf32>
            %48 = affine.load %arg6[%arg9, %arg10] : memref<?x64xf32>
            %49 = arith.addf %47, %48 : f32
            affine.store %49, %arg4[%arg9, %arg10] : memref<?x64xf32>
          }
        }
      }
    }
    return
  }
}

