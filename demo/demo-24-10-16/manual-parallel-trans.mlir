module {
  func.func @matmul_on_memref(%arg0: memref<128x128xi32>, %arg1: memref<128x128xi32>) -> memref<128x128xi32> {
    ...
    %group1 = async.create_group %c4 : !async.group
    ...
    scf.parallel (%arg2, %arg3) = (%c0, %c0) to (%c128, %c128) step (%c64, %c64) {
      ...
      %5 = scf.for %arg4 = %c0_2 to %c128_3 step %c64_4 iter_args(%arg5 = %4) -> (!air.async.token) {
        ...
        %group2 = async.create_group %c4 : !async.group
        scf.parallel (%arg6, %arg7) = (%c0_5, %c0_5) to (%c64_6, %c64_6) step (%c32_7, %c32_7) {
          ...
          %token2 = builtin.unrealized_conversion_cast %12: !air.async.token to !async.token
          async.add_to_group %token2, %group2 : !async.token
        }
        %group2wait = async.execute {
          async.await_all %group2
          async.yield
        }
        %8 = builtin.unrealized_conversion_cast %group2wait: !async.token to !air.async.token
      ...
      %token1 = builtin.unrealized_conversion_cast %6: !air.async.token to !async.token
      async.add_to_group %token1, %group1 : !async.token
    }

    %group1wait = async.execute {
