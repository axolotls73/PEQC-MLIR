...
//
// broadcast-basic.mlir: This file is part of the PEQC-MLIR project.
//
...
// RUN: %pastchecker %t/translation.c %t/translation.c aie_buffer_a34,aie_buffer_a44 | grep YES

// RUN: %pastchecker %t/translation.c %t/compare.c aie_buffer_a34,aie_buffer_a44 | grep YES
...
module @tutorial_4 {
    aie.device(xcvc1902) {
        ...
        %buf44 = aie.buffer(%tile44) { sym_name = "a44" } : memref<16xi32>

        aie.objectfifo @of (%tile14, {%tile34, %tile44}, 1 : i32) : !aie.objectfifo<memref<16xi32>>
        ...
        %core44 = aie.core(%tile44) {
            ...
            memref.copy %input, %buf44 : memref<16xi32> to memref<16xi32>
...
int* aie_buffer_a34;
{
    for (int i = 0; i < 16; i++) {
        ...
        aie_buffer_a44[i] = aie_buffer_a14[i];
    }
}
